...
public interface ITaskWebApiService
{
    ...
    Task<(IReadOnlyList<TaskItem> Items, int Total)> GetAssignedAsync(string? status, string sortBy, string order, int page, int pageSize, CancellationToken ct = default);

    Task<(IReadOnlyList<TaskItem> Items, int Total)> SearchAsync(TaskSearchQuery query, CancellationToken ct = default);
}
